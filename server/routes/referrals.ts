...
router.get("/:id/pdf", async (req, res) => {
  try {
    ...
    // Get company information for multi-tenant header
    ...
    generateReferralPDF(doc, referral, company);
...
function generateReferralPDF(doc: any, referral: any, company: any): void {
  ...
  const addFooter = () => {
    ...
    doc.text(`Generated by ${company.name}`, margin, pageHeight - 22);
  ...
  // Company name and details (multi-tenant)
  doc.setTextColor('#FFFFFF');
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(14);
  ...
  if (company.businessAddress) {
    const addressLines = company.businessAddress.split('\n');
    let addressY = 28;
  ...
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(16);
  doc.text('NDIS PARTICIPANT REFERRAL', margin, 50);
  doc.setFontSize(11);
  doc.text('Professional Assessment & Management Report', margin, 62);
