...
Maximum 400 words.`;
  ...
  // Get timesheet entries for detailed view
  app.get("/api/admin/timesheets/:timesheetId/entries", requireAuth, requireRole(["Admin", "ConsoleManager"]), async (req: any, res) => {
    try {
      const timesheetId = parseInt(req.params.timesheetId);
      ...
      const entries = await db
        .select({
          ...
          entryDate: timesheetEntriesTable.entryDate,
          startTime: timesheetEntriesTable.startTime,
          endTime: timesheetEntriesTable.endTime,
          breakMinutes: timesheetEntriesTable.breakMinutes,
          totalHours: timesheetEntriesTable.totalHours,
          hourlyRate: timesheetEntriesTable.hourlyRate,
          grossPay: timesheetEntriesTable.grossPay,
          isAutoGenerated: timesheetEntriesTable.isAutoGenerated,
          notes: timesheetEntriesTable.notes,
          shiftId: timesheetEntriesTable.shiftId,
          shiftTitle: shiftsTable.title,
          shiftDescription: shiftsTable.description,
          clientName: clients.fullName,
          clientId: clients.id,
        })
        .from(timesheetEntriesTable)
        .leftJoin(shiftsTable, eq(timesheetEntriesTable.shiftId, shiftsTable.id))
        .leftJoin(clients, eq(shiftsTable.clientId, clients.id))
        .where(and(
          eq(timesheetEntriesTable.timesheetId, timesheetId),
          eq(timesheetEntriesTable.tenantId, req.user.tenantId)
        ))
        .orderBy(desc(timesheetEntriesTable.entryDate));
