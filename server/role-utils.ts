/**
 * Server-side standardized role checking utilities for consistent case-insensitive role management
 */
 ...
 */
export function canManageCompanies(userRole: string | undefined | null): boolean {
  return hasRole(userRole, 'ConsoleManager');
}

/**
 * Check if user can view billing analytics
 */
export function canViewBilling(userRole: string | undefined | null): boolean {
  return hasAnyRole(userRole, ['Admin', 'TeamLeader', 'Coordinator', 'ConsoleManager']);
}

/**
 * Server-side role validation middleware factory
 */
export function requireRole(allowedRoles: UserRole[]) {
  return (req: any, res: any, next: any) => {
    console.log(`[ROLE CHECK] User role: ${req.user.role}, Required roles: ${allowedRoles.join(', ')}`);
    
    const userRole = req.user.role;
    
    // ConsoleManager has universal access
    if (hasRole(userRole, 'ConsoleManager')) {
...
/**
 * Role display names for server logging
 */
