...
interface MealtimeSectionCompleteProps {
  data: any;
...
export function MealtimeSectionComplete({ data, updateData, selectedClient, planData }: MealtimeSectionCompleteProps) {
  const { toast } = useToast();
  const mealtimeData: MealtimeData = data?.mealtimeData || {
    userInput: '',
    generatedContent: '',
    riskParameters: [],
    dietaryRequirements: '',
  ...
  const refreshGPTLimit = () => {
    console.log("GPT limit refreshed for next mealtime generation");
  };
  ...
  const handleInputChange = (field: string, value: string) => {
    ...
    updateData('mealtimeData', updatedData);
  };
  ...
  const removeRisk = (riskId: string) => {
    const updatedRisks = mealtimeData.riskParameters.filter(r => r.id !== riskId);
    const updatedData = {
      ...mealtimeData,
      riskParameters: updatedRisks
    };
  ...
  const generateContentMutation = useMutation({
    mutationFn: async ({ targetField }: { targetField: string }) => {
      ...
      // Include risk parameters for context
      const riskContext = mealtimeData.riskParameters.length > 0 
        ? `Current risk parameters: ${mealtimeData.riskParameters.map(r => `${r.risk} (${r.severity} severity)`).join('; ')}`
        ...
        <CardContent className="space-y-6">
          ...
          <div className="space-y-3">
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400">
              After initial generation, use these buttons to add targeted content to specific mealtime areas.
            </p>
            <div className="grid grid-cols-2 gap-2">
              ...
              <Button 
                variant="outline" 
                size="sm"
                onClick={() => handleGenerateTargetedContent('monitoringRequirements')}
                disabled={isGenerating || !mealtimeData.userInput?.trim()}
                className="flex items-center gap-2"
              >
                <Sparkles className="h-4 w-4" />
          ...
          {isGenerating && (
            <div className="flex items-center gap-2 p-3 bg-blue-50 dark:bg-blue-950 rounded-lg border border-blue-200 dark:border-blue-800">
              ...
              <span className="text-sm text-blue-700 dark:text-blue-300">Generating targeted mealtime content with context awareness...</span>
            </div>
          )}
                ...
                <Button 
                  onClick={() => {
                    ...
                    toast({
                      ...
                      description: "AI-generated content has been added to Dietary Requirements field. GPT limit refreshed.",
                    });
                  }}
                  size="sm"
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                <Button 
                  onClick={() => {
                    ...
                    toast({
                      title: "Content Applied",
                      description: "Added to Dietary Requirements field. GPT limit refreshed.",
                    });
                  }}
                  variant="outline" 
                  size="sm"
                ...
                <Button 
                  onClick={() => {
                    handleInputChange("textureModifications", mealtimeData.generatedContent || "");
                    handleInputChange('generatedContent', '');
                    refreshGPTLimit();
                    toast({
                      title: "Content Applied", 
                      description: "Added to Texture Modifications field. GPT limit refreshed.",
                    });
                  }}
                  variant="outline" 
                  size="sm"
                ...
                <Button 
                  onClick={() => {
                    handleInputChange("assistanceLevel", mealtimeData.generatedContent || "");
                    handleInputChange('generatedContent', '');
                    refreshGPTLimit();
                    toast({
                      title: "Content Applied", 
                      description: "Added to Assistance Level field. GPT limit refreshed.",
                    });
                  }}
                  variant="outline" 
                  size="sm"
                ...
                <Button 
                  onClick={() => {
                    ...
                    toast({
                      title: "Content Applied", 
                      description: "Added to Emergency Procedures field. GPT limit refreshed.",
                    });
                  }}
                  variant="outline" 
                  size="sm"
                ...
                <Button 
                  onClick={() => {
                    handleInputChange("monitoringRequirements", mealtimeData.generatedContent || "");
                    handleInputChange('generatedContent', '');
                    refreshGPTLimit();
                    toast({
                      title: "Content Applied", 
                      description: "Added to Monitoring Requirements field. GPT limit refreshed.",
                    });
                  }}
                  variant="outline" 
                  size="sm"
          ...
          {mealtimeData.riskParameters.length > 0 && (
            <div className="border rounded-lg">
              <Table>
                ...
                <TableBody>
                  {mealtimeData.riskParameters.map((risk) => (
